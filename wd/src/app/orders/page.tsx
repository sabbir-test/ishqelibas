...
export default function OrdersPage() {
  ...
  const loadOrders = async () => {
    ...
    try {
      ...
      console.log('🔄 Loading orders for user:', authState.user.email)
      ...
      if (response.ok) {
        ...
        if (transformedOrders.length > 0) {
          console.log(`📋 Loaded ${transformedOrders.length} orders for ${authState.user.email}`)
        } else {
          console.log(`📭 No orders found for ${authState.user.email}`)
        }
      } else {
                ...
                {orders.length === 0 && (
                  <div className="space-y-4">
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                      <h4 className="font-semibold text-blue-900 mb-2">Why don't I see my orders?</h4>
                      <ul className="text-sm text-blue-700 text-left space-y-1">
                        <li>• Make sure you're logged in with the correct account</li>
                        <li>• Orders may take a few minutes to appear after placement</li>
                        <li>• Demo/test orders are automatically filtered out</li>
                        <li>• Check if your order was successfully completed</li>
                      </ul>
                    </div>
                    <Link href="/shop">
