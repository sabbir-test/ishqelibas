...
export default function AdminCustomOrdersPage() {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: `Failed to fetch custom orders: ${error.message}`,
        variant: "destructive"
      })
    } finally {
      setIsLoading(false)
  ...
  const handleViewDetails = (order: CustomOrder) => {
    try {
      // Validate order data before opening dialog
      if (!order || !order.id) {
        toast({
          title: "Error",
      ...
      // Ensure measurements is valid JSON or provide fallback
      const safeOrder = {
        ...order,
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  {selectedOrder.userMeasurements ? (
                    <div className="space-y-4">
                      ...
                      {/* Notes section */}
                      {selectedOrder.userMeasurements.notes && (
                        <div className="mt-4 pt-4 border-t">
                          <span className="font-medium">Notes:</span>
                          <p className="mt-1 text-gray-600 text-sm">{selectedOrder.userMeasurements.notes}</p>
                        </div>
                      )}
                      
                      {/* Measurement metadata */}
                      <div className="mt-4 pt-4 border-t text-xs text-gray-500">
                        <p>Last updated: {new Date(selectedOrder.userMeasurements.updatedAt).toLocaleDateString()}</p>
                      </div>
                    </div>
                  ) : (
                    <div className="text-center py-6">
                      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                        <div className="flex items-center justify-center mb-2">
                          ...
                          <span className="font-medium text-yellow-800">Measurement Status: Pending</span>
                        </div>
                        <p className="text-yellow-700 text-sm">
                          Measurement not updated in DB, it is pending.
                        </p>
                        <Button 
                          variant="outline" 
                          size="sm" 
                          ...
                          onClick={() => {
                            toast({
                              ...
                              description: "Please contact the customer to update their measurements"
                            })
                          }}
                        >
                          Update Measurements
